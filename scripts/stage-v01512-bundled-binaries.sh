#!/bin/sh
# Stage the two native outputs of the pinned v0.15.12 workflow into this plugin.
#
# This script deliberately accepts only the two GitHub Actions artifacts made by
# .github/workflows/build-bundled-binaries.yml for the released Tree Ring v0.15.12 tag.
...
X86_ARTIFACT=$1
ARM_ARTIFACT=$2
EXPECTED_TAG=v0.15.12
EXPECTED_VERSION='tree-ring 0.15.12'
...
verify_artifact() {
  ...
  require_provenance capture_command verified "$provenance"
  require_provenance dox_source_guard verified "$provenance"
  require_provenance agent_zero_activation verified "$provenance"
...
stage_dir=$(mktemp -d "${TMPDIR:-/tmp}/tree-ring-v01512-stage.XXXXXX")
cleanup() {
  rm -rf "$stage_dir"
}
