...
CORE_RELEASE_COMMIT = "e9433e281c9743988a194f5af36a8258f2b9cc68"
NATIVE_BUNDLE_RUN = "35026514548"
BOOKWORM_IMAGE = (
    "rust:1.95-bookworm"
...
def test_manifest_declares_rust_bridge_generation():
    ...
    assert defaults["cli"]["required_version"] == "0.15.12"
    ...
    assert capability == {
        ...
        "tree_ring_version": {"min": "0.15.12", "minor": "0.15"},
        "enabled": True,
    }
...
def test_release_docs_match_the_verified_v01512_bundle():
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    bundled = (ROOT / "bin" / "README.md").read_text(encoding="utf-8")
    normalized_readme = " ".join(readme.split())
    normalized_bundled = " ".join(bundled.split())

    assert "Plugin `3.4.2` targets the Tree Ring `0.15`" in readme
    assert "supports `tree-ring` `0.15.12` through `0.15.x`" in readme
    ...
    assert "without this plugin's activation contract cannot validate" in normalized_readme
    assert CORE_RELEASE_COMMIT in normalized_readme
    assert NATIVE_BUNDLE_RUN in normalized_readme
    assert "Plugin `3.4.2` requires Tree Ring `0.15.12` through `0.15.x`" in normalized_bundled
    assert CORE_RELEASE_COMMIT in normalized_bundled
    assert NATIVE_BUNDLE_RUN in normalized_bundled
    assert "immutable Tree Ring Memory tag `v0.15.12`" in bundled


def test_guarded_dox_minimum_matches_the_verified_bundle():
    defaults = yaml.safe_load((ROOT / "default_config.yaml").read_text(encoding="utf-8"))
    ...
    assert defaults["cli"]["required_version"] == "0.15.12"
    assert capability["tree_ring_version"]["min"] == "0.15.12"
    assert bundled_versions == {"tree-ring 0.15.12"}


def test_v01512_bundle_workflow_is_manual_and_resolves_the_release_tag_at_runtime():
    workflow = (ROOT / ".github" / "workflows" / "build-bundled-binaries.yml").read_text(
        encoding="utf-8"
    )
    stager_path = ROOT / "scripts" / "stage-v01512-bundled-binaries.sh"
    ...
    assert "TREE_RING_RELEASE_TAG: v0.15.12" in workflow
    assert "TREE_RING_RELEASE_VERSION: 0.15.12" in workflow
    ...
    assert "--test agent_zero_342_acceptance" in workflow
    assert "-p tree-ring-memory-sqlite --lib dox" in workflow
    assert "actions::adapters::tests" in workflow
    assert "tree-ring capture --help" in workflow
    assert 'echo "capture_command=verified"' in workflow
    assert "tree-ring-v0.15.12-${{ matrix.target }}" in workflow
    assert "v0.13.0" not in workflow
    assert "v0.15.4" not in workflow
    assert "EXPECTED_TAG=v0.15.12" in stager
    assert "EXPECTED_VERSION='tree-ring 0.15.12'" in stager
    assert "v0.15.4" not in stager
    assert "require_provenance capture_command verified" in stager
    assert "require_provenance dox_source_guard verified" in stager
    assert "require_provenance agent_zero_activation verified" in stager
    assert "verify-activation-pair.py upstream/target/release/tree-ring" in workflow
...
def test_bundled_linux_binaries_have_v01512_native_build_provenance():
    expected = {
    ...
    for target, (runner, machine) in expected.items():
        ...
        assert provenance["source_tag"] == "v0.15.12"
        ...
        assert provenance["binary_version"] == "tree-ring 0.15.12"
        assert provenance["capture_command"] == "verified"
        assert provenance["dox_source_guard"] == "verified"
        assert provenance["agent_zero_activation"] == "verified"
