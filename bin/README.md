...
Plugin `3.4.0` requires Tree Ring `0.15.5` through `0.15.x` because automatic
capture depends on `tree-ring capture`. These executables are built from
immutable Tree Ring Memory tag `v0.15.5`, commit
`ca17b0ec984dac9367561be97fa488ccd52ec594`, using the locked dependency graph
and pinned
...
Ring 0.15.5 bundled binaries** [workflow run
33147052949](https://github.com/TerminallyLazy/tree-ring-memory-agent-zero/actions/runs/33147052949).
...
Manually run the pinned **Prepare Tree Ring 0.15.5 bundled binaries** GitHub
...
- `tree-ring-v0.15.5-linux-x86_64`
- `tree-ring-v0.15.5-linux-aarch64`
...
```bash
scripts/stage-v0155-bundled-binaries.sh \
  /absolute/path/to/tree-ring-v0.15.5-linux-x86_64 \
  /absolute/path/to/tree-ring-v0.15.5-linux-aarch64
sha256sum -c bin/SHA256SUMS
# On macOS: shasum -a 256 -c bin/SHA256SUMS
```

The staging command accepts only regular, checksum-matching files with
`v0.15.5` provenance, the pinned Bookworm image, the expected native runner and
machine, `tree-ring 0.15.5`, and the same resolved core commit for both
