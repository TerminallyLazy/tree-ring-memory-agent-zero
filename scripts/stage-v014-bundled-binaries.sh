#!/bin/sh
# Stage the two native outputs of the pinned v0.14.0 workflow into this plugin.
#
# This script deliberately accepts only the two GitHub Actions artifacts made by
# .github/workflows/build-bundled-binaries.yml after Tree Ring v0.14.0 exists.
...
X86_ARTIFACT=$1
ARM_ARTIFACT=$2
EXPECTED_TAG=v0.14.0
EXPECTED_VERSION='tree-ring 0.14.0'
...
stage_dir=$(mktemp -d "${TMPDIR:-/tmp}/tree-ring-v014-stage.XXXXXX")
cleanup() {
  rm -rf "$stage_dir"
}
