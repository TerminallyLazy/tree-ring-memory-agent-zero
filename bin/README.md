...
Manually run the pinned **Prepare Tree Ring 0.15.4 bundled binaries** GitHub
...
- `tree-ring-v0.15.4-linux-x86_64`
- `tree-ring-v0.15.4-linux-aarch64`
...
```bash
scripts/stage-v0154-bundled-binaries.sh \
  /absolute/path/to/tree-ring-v0.15.4-linux-x86_64 \
  /absolute/path/to/tree-ring-v0.15.4-linux-aarch64
sha256sum -c bin/SHA256SUMS
# On macOS: shasum -a 256 -c bin/SHA256SUMS
```

The staging command accepts only regular, checksum-matching files with
`v0.15.4` provenance, the pinned Bookworm image, the expected native runner and
machine, `tree-ring 0.15.4`, and the same resolved core commit for both
