...
STAGER = ROOT / "scripts" / "stage-v0156-bundled-binaries.sh"
BOOKWORM_IMAGE = (
    "rust:1.95-bookworm"
...
def _artifact(
    ...
    *,
    ...
    machine: str,
    source_commit: str,
    source_tag: str = "v0.15.6",
    release_version: str = "0.15.6",
    capture_verified: bool = True,
) -> Path:
    root.mkdir()
    binary = root / "tree-ring"
    binary.write_text(
        f"#!/bin/sh\necho 'tree-ring {release_version}'\n", encoding="utf-8"
    )
    binary.chmod(0o755)
    ...
    provenance = [
        ...
        f"binary_version=tree-ring {release_version}",
    ]
    if capture_verified:
...
def test_release_stager_requires_matched_verified_v0156_artifacts(tmp_path):
...
def test_release_stager_refuses_the_checked_in_v0154_generation(tmp_path):
    plugin = _plugin_copy(tmp_path)
    source_commit = "a" * 40
    x86 = _artifact(
        tmp_path / "x86",
    ...
    assert result.returncode != 0
    assert "expected source_tag=v0.15.6" in result.stderr
    assert (plugin / "bin" / "linux-x86_64" / "tree-ring").read_bytes() == b"old-x86"
    assert (plugin / "bin" / "linux-aarch64" / "tree-ring").read_bytes() == b"old-arm"


def test_release_stager_requires_capture_command_provenance(tmp_path):
    plugin = _plugin_copy(tmp_path)
    source_commit = "a" * 40
    x86 = _artifact(
        tmp_path / "x86",
