...
CORE_RELEASE_COMMIT = "8fba7f31cd2036864e31dc3183d0ae220ef5d48b"
BOOKWORM_IMAGE = (
    "rust:1.95-bookworm"
...
def test_manifest_declares_rust_bridge_generation():
    ...
    assert defaults["cli"]["required_version"] == "0.15.4"
...
def test_release_docs_match_v0154_bundle():
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    bundled = (ROOT / "bin" / "README.md").read_text(encoding="utf-8")

    assert "Plugin `3.3.1` targets the released Tree Ring `0.15`" in readme
    assert "supports `tree-ring` `0.15.4` through `0.15.x`" in readme
    ...
    assert "Plugin `3.3.1` requires Tree Ring `0.15.4` through `0.15.x`" in bundled
    assert "immutable Tree Ring Memory tag `v0.15.4`" in bundled


def test_v0154_bundle_workflow_is_manual_and_resolves_the_release_tag_at_runtime():
    workflow = (ROOT / ".github" / "workflows" / "build-bundled-binaries.yml").read_text(
        encoding="utf-8"
    )
    stager = (ROOT / "scripts" / "stage-v0154-bundled-binaries.sh").read_text(
        encoding="utf-8"
    )

    assert "workflow_dispatch:" in workflow
    assert "pull_request:" not in workflow
    assert "TREE_RING_RELEASE_TAG: v0.15.4" in workflow
    assert "TREE_RING_RELEASE_VERSION: 0.15.4" in workflow
    ...
    assert "tree-ring-v0.15.4-${{ matrix.target }}" in workflow
    assert "v0.13.0" not in workflow
    assert "EXPECTED_TAG=v0.15.4" in stager
    assert "EXPECTED_VERSION='tree-ring 0.15.4'" in stager
...
def test_bundled_linux_binaries_have_v0154_native_build_provenance():
    expected = {
    ...
    for target, (runner, machine) in expected.items():
        ...
        assert provenance["source_tag"] == "v0.15.4"
        ...
        assert provenance["binary_version"] == "tree-ring 0.15.4"
