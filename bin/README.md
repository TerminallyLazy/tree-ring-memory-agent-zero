...
Plugin `3.4.1` requires Tree Ring `0.15.6` through `0.15.x` because automatic
capture depends on `tree-ring capture`. These executables are built from
immutable Tree Ring Memory tag `v0.15.6`, commit
`c807d3f507bf15300e4f40449acca40001a6121e`, using the locked dependency graph
and pinned
...
Ring 0.15.6 bundled binaries** [workflow run
...
Manually run the pinned **Prepare Tree Ring 0.15.6 bundled binaries** GitHub
...
- `tree-ring-v0.15.6-linux-x86_64`
- `tree-ring-v0.15.6-linux-aarch64`
...
```bash
scripts/stage-v0156-bundled-binaries.sh \
  /absolute/path/to/tree-ring-v0.15.6-linux-x86_64 \
  /absolute/path/to/tree-ring-v0.15.6-linux-aarch64
sha256sum -c bin/SHA256SUMS
# On macOS: shasum -a 256 -c bin/SHA256SUMS
```

The staging command accepts only regular, checksum-matching files with
`v0.15.6` provenance, the pinned Bookworm image, the expected native runner and
machine, `tree-ring 0.15.6`, and the same resolved core commit for both
